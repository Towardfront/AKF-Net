...
def collect_r2c_data(data_root, mode='train', record_file='./data/refsplits.json'):
    ...
    for c_idx in tqdm(range(len(camo_classes))):
        ...
        ref_cate_names = os.listdir(ref_cate_feats_dir)
        ...
        class_file_list[cate] = [os.path.join(ref_cate_feats_dir, ref_cate_names[f_idx]) for f_idx in range(len(ref_cate_names))]
