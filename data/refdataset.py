...
from .yg_utils import collect_r2c_data as collect
...
class R2CObjData(Dataset):
    ...
    def __init__(self, data_root, mode='train', shot=5, image_size=352):
        ...
        self.shot = shot
        #################################################################################################
        # 原方法为collect_r2c_data
        self.data_list, self.class_file_list = collect(data_root=self.data_root, mode=self.mode)
    ...
    def __getitem__(self, index):
        ...
        # 如：/data3/YG/underwater-dataset/Camo/train/Images/BatFish/COD10K-CAM-1-Aquatic-1-BatFish-2.jpg
        # 即 COD10K-CAM-2-Terrestrial-21-Ant-1234
        ...
        if self.shot > 0 or self.shot == -1: 
            ...
            ###################################################################################################
            class_chosen = image_path.split('/')[-2]


            # 如：RefCOD/R2C7K/Ref/RefFeat_ICON-R/Ant/320365802_0b8992f24d_o.npy
            # file_class_chosen 为 list
            ...
            if self.mode == 'train':
                # 参考图片采样序列号
                ref_idx_list = random.sample(range(num_aux), self.shot) if self.shot > 0 else list(range(num_aux))   # num_aux > shot 保证采样数量大于样本数量
            else:
        ...
        return image, label, sal_f, name, c
