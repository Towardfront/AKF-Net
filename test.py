...
def test_model(test_loader, model):
    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader)) as pbar:
            for (image, gt, sal_f, _, _) in test_loader:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='Net_epoch_best')
    ...
    parser.add_argument('--save_root', type=str, default='clutter_info', help='the path to save model params and log')

    opt = parser.parse_args()
    print(opt)

    # load model 
    ref_model = Network(channel=opt.dim).cuda()

    params_path = 'clutter_info/ckpt/sam_dino_backbonesaved_models/r2cnet/Net_epoch_best.pth'
