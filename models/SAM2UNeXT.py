...
import timm
from utils.build_sam import build_sam2
from timm.models.layers import trunc_normal_   
import sys
# You need to git clone https://github.com/WZH0120/SAM2-UNeXT.git and modify the corresponding paths accordingly.
sys.path.append("../SAM2-UNeXT-main")
...
class SAM2UNeXT(nn.Module):
    def __init__(self, checkpoint_path=None, dinov2_path=None) -> None:
        ...
        # ===== SAM2 Encoder =====    
        model_cfg = "../SAM2-UNeXT-main/sam2_configs/sam2_hiera_l.yaml"
        if checkpoint_path:
...
if __name__ == '__main__':
    ...
    model = SAM2UNeXT(checkpoint_path=r'ckpt\sam2_hiera_large.pt', dinov2_path=r'ckpt\model.safetensors')
    model = model.to(device)

    input = torch.randn((2, 3, 384, 384)).to(device)

    out1 = model.sam(input)
    out2 = model.dino(F.interpolate(input, size=(448, 448), mode='bilinear'))
    print(out1.shape)
    for idx, item in enumerate(out2):
