...
class QuadrangleAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=True, attn_drop=0., proj_drop=0., window_size=7, coords_lambda=20):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        self.transform = nn.Sequential(
            ...
            nn.Conv2d(dim, num_heads * 2, kernel_size=1)  # 只预测平移参数 (dx, dy)
        )
    ...
    def forward(self, x, H, W):
        ...
        qkv = self.qkv(x.flatten(2).transpose(1, 2))  # B,HW,3C
        qkv = qkv.reshape(B, H*W, 3, self.num_heads, C//self.num_heads).permute(2,0,3,1,4)
        q, k, v = qkv[0], qkv[1], qkv[2]

        # 简单的坐标偏移
        offsets = self.transform(x)  # B, num_heads*2, H/ws, W/ws
        offsets = offsets.view(B, self.num_heads, 2, H//self.window_size, W//self.window_size)

        # 这里只演示逻辑：实际应使用 grid_sample
        # 为了保持兼容性，我们直接用原始 k,v
...
class SwinTransformerBlock_v2(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        elif attn_type == "quadrangle":
            self.attn = QuadrangleAttention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias,
                attn_drop=attn_drop, proj_drop=drop, window_size=self.window_size)
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load("/media/ubuntu/cdb918a9-e3b3-40a1-bf0e-97b8b44001ca/YG/RefCOD/pretrained/swin_base_patch4_window12_384.pth")['model'], strict=False)
